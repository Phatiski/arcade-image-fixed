...
namespace FxImage {
    ...
    export function startIndex(fximg: Buffer) {
        return getFximgOffset(fximg[0], 0x3).idx;
    }
    ...
    export function toImage(fximg: Buffer): Image {
        const img = image.create(widthOf(fximg), heightOf(fximg));
        const tmpn = img.height;
        const tbuf = pins.createBuffer(tmpn);
        for (let x = 0; x < img.width; x++) {
    ...
    export function toFrame(fximgs: Buffer) {
        const imgs: Image[] = []
        const img = image.create(widthOf(fximgs), heightOf(fximgs));
        const tmpn = img.height;
        const tbuf = pins.createBuffer(tmpn);
        const startIdx = startIndex(fximgs);
    ...
    export function setPixel(fximg: Buffer, x: number, y: number, c: number) {
        ...
        c &= 0xf;
        const i = _pos2idx(x, heightOf(fximg), y);
        const ih4 = (i >>> 1) + startIndex(fximg);
    ...
    export function getPixel(fximg: Buffer, x: number, y: number) {
        if (isOutOfArea(x, y, widthOf(fximg) * lenghtOf(fximg), heightOf(fximg))) return 0;
        const i = _pos2idx(x, heightOf(fximg), y);
        const ih = i >>> 1;
        const ih4 = ih + startIndex(fximg);
        const curv = fximg[ih4];
        return (i & 1 ? curv & 0xf : curv >>> 4)
    }

    export function setRow(fximg: Buffer, x: number, src: Buffer, tmpn?: number) {
        if (isOutOfRange(x, widthOf(fximg) * lenghtOf(fximg))) return;
        const h0 = tmpn ? tmpn : heightOf(fximg);
        const len = Math.min(src.length, h0);
        const start = startIndex(fximg);
        if (len < 1) return;
        let i = x * h0,
            y = 0;
        if (i & 1) {
    ...
    export function drawLine(fximg: Buffer, x0: number, y0: number, x1: number, y1: number, color: number) {
        ...
        const w = widthOf(fximg);
        const h = heightOf(fximg);
        color &= 0xF;
    ...
    function _bulitDrawImage(fromFximg: Buffer, toFximg: Buffer, dx: number, dy: number, transparent: boolean) {
        const sw = widthOf(fromFximg)
        const sh = heightOf(fromFximg);
        const tw = widthOf(toFximg)
        const th = heightOf(toFximg);
    ...
    export function scale(fximg: Buffer, newWidth: number, newHeight: number): Buffer {
        const ow = widthOf(fximg);
        const oh = heightOf(fximg);
